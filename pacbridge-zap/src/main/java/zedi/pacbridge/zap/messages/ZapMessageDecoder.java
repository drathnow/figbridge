...
public class ZapMessageDecoder {
    ...
    public String decodePacketBytes(ByteBuffer byteBuffer) {
        ...
        SessionHeader header = (SessionHeader)packet.getHeader();
    ...
    public String decodeMessageBytes(ZapMessageType messageType, ByteBuffer byteBuffer) {
        ...
        return "Message type not currently supported: '" + messageType + "'";
    }
    
    public String formattedMessage(Message message) {
        switch (message.messageType().getNumber()) {
            case ZapMessageType.BUNDLED_REPORT_NUMBER: return decodeMessage((BundledReportMessage)message);
            case ZapMessageType.HEART_BEAT_MESSAGE_NUMBER : return decodeMessage((HeartBeatMessage)message);
            case ZapMessageType.ACK_MESSAGE_NUMBER : return decodeMessage((AckMessage)message);
            case ZapMessageType.WRITE_IO_POINT_NUMBER : return decodeMessage((WriteIoPointsControl)message); 
            case ZapMessageType.DEMAND_POLL_NUMBER : return decodeMessage((DemandPollControl)message);
            case ZapMessageType.OTAD_REQUEST_NUMBER : return decodeMessage((OtadRequestControl)message);
            case ZapMessageType.OTAD_STATUS_UPDATE_NUMBER : return decodeMessage((OtadStatusMessage)message);
        }
    ...
    public String decodeMessage(ZapReport report) {
        ...
        for (Iterator<ReadingCollection> iterator = report.readingCollections().iterator();
                iterator.hasNext(); )
