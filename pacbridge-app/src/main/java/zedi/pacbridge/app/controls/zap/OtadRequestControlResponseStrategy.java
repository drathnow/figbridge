...
public class OtadRequestControlResponseStrategy extends BaseControlResponseStrategy implements ControlResponseStrategy, RequestCompletionStrategy  {
    private static final Logger logger = LoggerFactory.getLogger(DemandPollControlResponseStrategy.class.getName());
    
    private boolean responseSent;
    
    public OtadRequestControlResponseStrategy(OtadRequestControl forControl, SiteAddress siteAddress, EventHandler eventPublisher, InterestingSitesCache interestingSitesCache) {
    ...
    public void handleMessage(Message message) {
        if (message.messageType() == ZapMessageType.Acknowledgement) {
            ...
            if (isExpectedMessage(ackMessage) && isNotProtocolError(ackMessage)) {
                OtadRequestAckDetails details = ackMessage.additionalDetails();
