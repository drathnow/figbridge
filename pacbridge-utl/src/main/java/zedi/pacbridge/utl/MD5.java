/*
 * Copyright (C) Wireless Matrix 2003 - . All Rights Reserved. The
 * software contained on this media is proprietary to and embodies the
 * confidential technology of Wireless Matrix, use, duplication or
 * dissemination of the software media is authorized pursuant to a
 * valid written license.
 */
///CLOVER:OFF
...
public class MD5 {
    ...
    private int[] Decode(byte buffer[], int len, int shift) {
        ...
        for (i = j = 0; j < len; i++, j += 4) {
            out[i] = ((int)(buffer[j + shift] & 0xff)) | (((int)(buffer[j + 1 + shift] & 0xff)) << 8) | (((int)(buffer[j + 2 + shift] & 0xff)) << 16) | (((int)(buffer[j + 3 + shift] & 0xff)) << 24);
        }
     ...
     */
    public synchronized byte[] finalValue() {
        byte bits[];
        int index, padlen;
        MD5State fin;
     ...
     *
     * @return This object's hash as a string.
     */
    public String asHex() {
        return asHex(this.finalValue());
    }
}